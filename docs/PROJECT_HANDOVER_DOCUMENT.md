...
- Multi-image upload with compression and caching to Amazon S3 ST bucket
...
The listing module integrates with Amazon S3 ST bucket for image persistence (images are NOT stored in Firebase Storage). Uploads use pre-signed URLs for direct S3 transfer, with CDN acceleration for fast image delivery.

### 3.3 Property Discovery and Search

The public listings feature enables Guests to discover available accommodations through an intuitive search and filtering interface. This module incorporates location-based search using stored latitude/longitude coordinates (no Google Maps integration), advanced filtering mechanisms, and comprehensive property detail presentation.

**User Experience Features:**
- Location-based search using latitude/longitude coordinates (no map view)
- Multi-criteria filtering (price, amenities, availability, distance radius)
- High-resolution image galleries with zoom capabilities (S3-hosted images)
...
- **Amazon S3 ST Bucket**: Image and file asset storage (no Firebase Storage)
- **Payment Gateway**: Secure transaction processing (implementation details confidential)
- **Location Services**: Property coordinates stored as latitude/longitude (no Google Maps)
