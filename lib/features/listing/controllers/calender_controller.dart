...
class CalendarController extends GetxController {
  ...
  final Rx<CalendarResponse?> calendarResponse = Rx<CalendarResponse?>(null);
  ...
  bool isBooked(DateTime date) {
    ...
    return dayData != null && dayData.isBooked!;
  }

  UserData? getBookingUser(DateTime date) {
    ...
    if (dayData != null && (dayData.isBooked ?? false)) {
      return dayData.bookingData?.user;
    }
    return null;
  }
  ...
  bool isBlocked(DateTime date) {
    ...
    final dateKey = DateFormat('yyyy-MM-dd').format(date);
    final dayData = calendarResponse.value!.data.calendarData[dateKey];
    return dayData != null && dayData.isBlocked!;
  }
  ...
  String getLabelForDate(DateTime date) {
    ...
    if (dayData.isBlocked!) return 'Blocked';
    if (dayData.price! > 0) return '৳ ${dayData.price!.toStringAsFixed(0)}';

    return '';
  }

  List<BookingPayload> generateBookingPayload({
    required double price,
    required String note,
  }) {
    final List<BookingPayload> result = [];
  ...
  List<BookingPayload> generateBlockedPayload(String? note) {
    final List<BookingPayload> result = [];
