...
class PublicAssistancePaymentScreen extends GetView<AssistanceBookingController> {

  static const String route = '/public_assistance_payment_screen';

   PublicAssistancePaymentScreen({super.key});


   final AssistanceBookingController _bookingController= Get.find<AssistanceBookingController>();
  ...
  Widget build(BuildContext context) {
    ...
    PublicAssistanceData? data = controller.assistanceData;
    double extraGuestCharge = (controller.assistanceData?.extraChargePerPerson?? 0).toDouble() * (controller.extraGuestCount ?? 0).toDouble();
    double total = ((data?.price ?? 0.0) * ((controller.nightCount ?? 0).toDouble())) +
        ((extraGuestCharge ?? 0.0) * (controller.nightCount ?? 0));

    double gatewayCharge = total * (data?.serviceChargePercentage ?? 0.0);
      ...
      body: ListView(
          ...
          Container(
            ...
            child: Column(
              children: [
                ...
                Divider(color: const Color(0xFFE6E8EE),thickness: 3, height: 25,),
                Padding(
                  padding: const EdgeInsets.symmetric(horizontal: 15),
                  child: Row(
                    mainAxisAlignment: MainAxisAlignment.spaceBetween,
        ...
        child: Column(
          children: [
            ...
            Padding(
              ...
              child: Row(
                ...
                style: TextStyle(
                  ...
                  Text(
                    '${(data?.price ?? 0.0) * (controller.nightCount ?? 0)} TK',
                    textAlign: TextAlign.center,
                    style: TextStyle(
                      color: const Color(0xFF505050) /* Gray */,
                      fontSize: 16.61,
            ...
            Divider(color: const Color(0xFFE6E8EE),thickness: 3, height: 25,),
            if((controller.extraGuestCount ?? 0) > 0)...[Padding(
              padding: const EdgeInsets.symmetric(horizontal: 15),
              child: Row(
                mainAxisAlignment: MainAxisAlignment.spaceBetween,
                children: [
                  Text(
                    'Extra guest ${controller.extraGuestCount ?? 0}',
                    textAlign: TextAlign.center,
                    style: TextStyle(
                      color: Colors.black /* Black */,
                      fontSize: 18.68,
                  ...
                  Text(
                    '${((controller.assistanceData?.extraChargePerPerson?? 0).toDouble() * (controller.extraGuestCount ?? 0).toDouble()) * (controller.nightCount ?? 0)} TK',
                    textAlign: TextAlign.center,
                    style: TextStyle(
                      color: const Color(0xFF505050) /* Gray */,
                      fontSize: 16.61,
            ...
            Obx(() {
                ...
                if(_bookingController.decrasePrice.value <= 0) {
                  return SizedBox();
                }
                return Column(
                  children: [
                    Padding(
                      ...
                      child: Row(
                        ...
                        children: [
                          ...
                          Text(
                            '${(_bookingController.decrasePrice.value).toStringAsFixed(2)} TK',
                            textAlign: TextAlign.center,
                            style: TextStyle(
                              color: const Color(0xFF505050) /* Gray */,
                              fontSize: 16.61,
            ...
            Padding(
              ...
              child: Row(
                ...
                children: [
                  ...
                  Obx(() {
                    return Text(
                      '${((total - _bookingController.decrasePrice.value) + gatewayCharge).toStringAsFixed(2)} TK',
                      textAlign: TextAlign.center,
                      style: TextStyle(
                        color: const Color(0xFF505050) /* Gray */,
                        fontSize: 20.76,
          ...
          Center(
            child: Obx(() => InkWell(
              onTap: () async {
                ...
                try {
                  final booking = {
                      "assistanceId": data?.id ?? 0,
                      "checkIn": formatDate(controller.startDate),
                      "checkOut": formatDate(controller.endDate),
                      "guestCount": data?.maxPerson ?? 1,
                      "extraGuestCount": controller.extraGuestCount ?? 1,
                  ...
                  if(data?.instantBookingAllowed==true){
                    if (result != null) {
                      ...
                      final paymentData = await controller.createPayment(paymentPayload);
                      if (paymentData != null) {
